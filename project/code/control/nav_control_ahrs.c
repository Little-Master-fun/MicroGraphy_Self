...
nav_ahrs_status_enum nav_ahrs_init(void)
{
    ...
    // 前瞻参数
    nav_ahrs.lookahead_distance = NAV_AHRS_DISTANCE_PER_POINT * 1; 
    ...
    nav_ahrs.stable_wait_required = NAV_AHRS_STABLE_WAIT_TIME_MS / NAV_AHRS_UPDATE_PERIOD_MS;  
...
{
    // 动态前瞻距离（根据速度和曲率调整）
    float speed_factor = nav_ahrs.base_speed / 2.5f;  
