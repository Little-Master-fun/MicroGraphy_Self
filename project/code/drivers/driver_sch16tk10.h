...
typedef struct _SCH1_spi_stats {
    uint32_t total_requests;           // 鎬昏?锋眰娆℃暟
    uint32_t successful_requests;      // 鎴愬姛璇锋眰娆℃暟
    uint32_t crc_errors;              // CRC閿欒??娆℃暟
    uint32_t frame_errors;            // 甯ч敊璇?娆℃暟
    uint32_t timeout_errors;          // 瓒呮椂閿欒??娆℃暟
    uint32_t value_inconsistencies;   // 鏁板�间笉涓�鑷存?℃暟
    uint32_t last_error_type;         // 鏈�鍚庝竴娆￠敊璇?绫诲瀷 (0=鏃犻敊璇?, 1=CRC, 2=甯?, 3=瓒呮椂, 4=涓嶄竴鑷?)
    uint32_t consecutive_errors;      // 杩炵画閿欒??娆℃暟
    uint32_t max_consecutive_errors;  // 鏈�澶ц繛缁?閿欒??娆℃暟
    float error_rate;                 // 閿欒??鐜? (鐧惧垎姣?)
    uint32_t last_test_timestamp;     // 鏈�鍚庢祴璇曟椂闂存埑
} SCH1_spi_stats;
...
// 硬件抽象层函数声明 / Hardware abstraction layer function prototypes
void        hw_init(void);                          // 硬件初始化 / Hardware initialization
void        hw_EXTRESN_High(void);                  // 外部复位信号拉高 / Set external reset signal high
void        hw_EXTRESN_Low(void);                   // 外部复位信号拉低 / Set external reset signal low
void        hw_CS_High(void);                       // 片选信号拉高 / Set chip select signal high
void        hw_CS_Low(void);                        // 片选信号拉低 / Set chip select signal low
void        hw_delay(uint32_t ms);                  // 毫秒延时函数 / Millisecond delay function
uint64_t    hw_SPI48_Send_Request(uint64_t Request);// 发送48位SPI请求 / Send 48-bit SPI request
void        hw_timer_setFreq(uint32_t freq);        // 设置定时器频率 / Set timer frequency
void        hw_timer_startIT(void);                 // 启动定时器中断 / Start timer interrupt
void        hw_timer_stopIT(void);                  // 停止定时器中断 / Stop timer interrupt

// 传感器驱动函数声明 / Sensor driver function prototypes
